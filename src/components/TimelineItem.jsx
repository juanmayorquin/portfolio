...
const TimelineItem = ({ title, school, initYear, finishYear, skills }) => {
  return (
    <div className="timeline-item flex items-center gap-5 group">
      ...
      <div className="timeline-item-content w-full flex flex-col gap-4 group-hover:bg-white/5 group-hover:translate-x-3 px-8 py-5 rounded-xl transition-all">
        <h3 className="text-white text-3xl font-semibold">{title}</h3>
        <p className="text-neutral-400 text-lg font-medium">{school}</p>
        <div className="flex flex-wrap gap-2">
            {skills && skills.map((skill, index) => <Tag name={skill} key={index}/>)}
        </div>
